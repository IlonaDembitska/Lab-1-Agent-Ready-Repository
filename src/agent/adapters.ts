...
async function postJson(
  ...
  if (!res.ok) {
    throw new Error(
      `${o.url} → HTTP ${res.status}: ${(await res.text()).slice(0, 500)}`,
    );
  }
  ...
  switch (m.role) {
    ...
    case 'assistant':
      return {
        role: 'assistant',
        content: [
          ...(m.text !== '' || m.calls.length === 0
            ? [
                {
                  type: 'text',
                  text: m.text || '(порожньо)',
                },
              ]
            : []),
...
export function messagesModel(
  ...
  return async (system, messages, tools) => {
    ...
    return {
      text,
      calls,
      usage: fromMessagesUsage(data.usage),
    };
  };
}
...
export function chatCompletionsModel(
  ...
  return async (system, messages, tools) => {
    ...
    if (message === undefined) {
      throw new Error(
        'Відповідь без choices[0].message',
      );
    }
    ...
    return {
      ...
      calls,
      usage: fromChatUsage(data.usage),
    };
  };
}