...
export type LoopResult<T> =
  ...
  | {
      readonly stop: 'max-steps' | 'token-budget';
      readonly steps: number;
      readonly usage: Usage;
    };
...
export async function runAgentLoop<T>(
  ...
  for (let step = 1; step <= o.maxSteps; step++) {
    ...
    if (turn.calls.length === 0) {
      const parsed = o.output.safeParse(
        extractJson(turn.text),
      );
    ...
    for (const call of turn.calls) {
      ...
      const { outcome, content } = tool
        ...
        : {
            ...
            content: `Невідомий інструмент: ${call.name}`,
          };
